...
(define-param chi_deg (* 0.95 (Critical n1 n2))) ; incidence angle in degrees
(define-param krw 0.3) ; beam waist distance to interface (dimensionless)
(define-param kr_c 5.0) ; radius of curvature (dimensionless)
...
(set! geometry (list
				(make cylinder
				(center (* kr_c (cos (chi_rad chi_deg))) (* -1.0 (* kr_c (sin (chi_rad chi_deg))))) 
...
(define (Complexfactor k x sigma)
		(+ 1.0 (/ (* 0+1i (vector3-x x)) (* pi (vector3-norm k) sigma sigma))))

(define ((Gaussian sigma k) x)
		(* (/ 1.0 (sqrt (Complexfactor k x sigma))) (exp (- (* 0+2i pi (vector3-dot k x))
				(/(/ (vector3-dot x x) (* sigma sigma)) (Complexfactor k x sigma))))))
